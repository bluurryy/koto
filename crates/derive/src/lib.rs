...
assert_valid_features! {
    ...
    ///
    /// If the `"gc"` or `"agc"` feature is enabled, this macro will produce an implementation of
    ...
    ///
    /// Since `KotoTrace` is implemented for all types that satisfy its bounds, this automatically
    /// implements `KotoTrace` for the type, provided it is also `'static`.
    ///
    ...
    /// `#[koto(trace(ignore))]` attribute them or the whole container.
    /// This is useful for types of third party crates that you can't implement the trace trait for.
    ///
